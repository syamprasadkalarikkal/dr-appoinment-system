...
export default function PatientDashboard() {
  ...
  const upcomingAppointments = appointments.filter(apt => 
    apt.status === 'scheduled' && new Date(apt.time_slot?.date) >= new Date()
  );

  const pastAppointments = appointments.filter(apt => 
    apt.status === 'completed' || new Date(apt.time_slot?.date) < new Date()
  );
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
              <div className="border-b border-gray-200">
                <div className="flex space-x-8 px-6">
                  ...
                  <button className="py-4 border-b-2 border-transparent text-gray-500 hover:text-gray-700 font-semibold">
                  ...
                  {specialties.map(specialty => (
                    ...
                    >
                      ...
                      <div className={`text-xs font-semibold ${
                        selectedSpecialty === specialty.id ? 'text-blue-700' : 'text-gray-700'
                      }`}>
                        {specialty.name}
                      </div>
                    </button>
